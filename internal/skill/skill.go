...
- A run object may include ` + "`skipped_steps`" + `, naming the steps this run will never execute, or ` + "`none`" + ` when it skips nothing. Read it before you approve a gate: a run that skips ` + "`push`" + `, ` + "`pr`" + ` and ` + "`ci`" + ` cannot leave this machine no matter how you answer, while a run that skips none of them can still reach them. A step this run will skip stays ` + "`pending`" + ` in the ` + "`steps`" + ` table until the run reaches it, so the table alone cannot tell you which it is. Runs created before the pipeline recorded this omit the field, which means unknown rather than nothing skipped.
