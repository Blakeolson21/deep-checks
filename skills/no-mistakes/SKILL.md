...
- A run object may include `skipped_steps`, naming the steps this run will never execute, or `none` when it skips nothing. Read it before you approve a gate to know whether finishing the run pushes the branch and opens a PR, or ends on this machine. A step this run will skip still reads `pending` in the `steps` table until the run reaches it, so the table alone cannot answer that. Runs created before the pipeline recorded this omit the field, which means unknown rather than nothing skipped.
