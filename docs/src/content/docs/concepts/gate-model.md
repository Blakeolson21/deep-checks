...
Each run also records `skipped_steps`, the steps it will never execute, written when the run is created so the consequence of answering a gate is knowable before those steps are reached, and read back on daemon restart so a resumed run honours the plan it started with.
An empty value means the run explicitly skips nothing and therefore reaches push; `NULL` means the run predates the column and its plan is unknown, which readers must not report as skipping nothing.
